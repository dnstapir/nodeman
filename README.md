...
Each DNS TAPIR node has several credentials:

- The _enrollment key_ is a single-use key used for initial enrollment. It's a JWK generated by the DNS TAPIR cluster administrator and supplied to the edge administrator. An enrollment key may have a limited lifetime.

- The _data key_ is used to sign submitted data. It's JWK (default OKP/Ed25519 with signature algorithm EdDSA) generated by the edge node and is submitted to the node manager as part of the initial enrollment. The data key will never expire and cannot be revoked; if lost the node has to be deleted and re-enrolled as a new node.

- The _node certificate_ is an X.509 certificate (default Ed25519) issued by the node manager CA and is used for TLS client authentication when connecting to the DNS TAPIR cluster. It is valid for a relative short time (default 90 days) and must be periodically renewed by the edge node. Renewals are authenticated by the node's _data key_.  Node certificates are not revoked when the node is deleted.
