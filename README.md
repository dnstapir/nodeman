...
- The _data key_ is used to sign submitted data. It's JWK (default EdDSA/Ed25519) generated by the edge node and submitted to the node manager as part of the initial enrollment. The data key will never expire; if lost the node has to be re-enrolled as a new node.

- The _node certificate_ is a X.509 certificate used for authentication TLS connections the DNS TAPIR cluster and is issued by the node manager CA. It is valid for a relative short time (default 90 days) and must be periodically renewed by the edge administrator. Renewals are authenticated by the node's _data key_.
